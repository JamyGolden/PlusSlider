...
base.$sliderControls.find('a').click(function(){base.toSlide($(this).attr('rel'));return false;});base.$sliderControls.find('a').eq(0).addClass('current');}
base.nextSlide=function(direction){if(direction!==false){direction=true;}
if(base.options.sliderType=='slider'){if(direction==true&&!base.$el.is(':animated')){if(base.$el.position().left>(base.$stopPosition-1)*-1){if(base.options.createPagination){base.$sliderControls.find('a.current').removeClass('current').next().addClass('current');}
base.$el.animate({left:"-="+base.$slideWidth+"px"},base.options.speed,base.options.sliderEasing);base.$el.children('.current').removeClass('current').next().addClass('current');}else{if(base.options.createPagination){base.$sliderControls.find('a:last').removeClass('current').siblings('a:first').addClass('current');}
base.$el.animate({left:0},base.options.speed,base.options.sliderEasing);base.$el.children('.current').removeClass('current').siblings(':first').addClass('current');}}else if(direction==false&&!base.$el.is(':animated')){if(base.$el.position().left<0&&!base.$slides.is(':animated')){if(base.options.createPagination){base.$sliderControls.find('a.current').removeClass('current').prev().addClass('current');}
base.$el.animate({left:"+="+base.$slideWidth+"px"},base.options.speed,base.options.sliderEasing);base.$el.children('.current').removeClass('current').prev().addClass('current');}else{if(base.options.createPagination){base.$sliderControls.find('a:first').removeClass('current').siblings('a:last').addClass('current');}
base.$el.animate({left:"-="+base.$stopPosition+"px"},base.options.speed,base.options.sliderEasing);base.$el.children('.current').removeClass('current').siblings(':last').addClass('current');}}}else{if(direction==true&&!base.$slides.is(':animated')){if(base.$el.children(':last').is('.current')){if(base.options.createPagination){base.$sliderControls.find('a:last').removeClass('current').siblings('a:first').addClass('current');}
...
if(base.options.onSlide&&typeof(base.options.onSlide)=="function"){base.options.onSlide(base.$slides.filter('.current').index());}}
base.toSlide=function(index){if(!base.$slides.is(':animated')){if(base.options.createPagination){base.$sliderControls.find('a[rel="'+index+'"]').addClass('current').siblings().removeClass('current');}
if(base.options.sliderType=='slider'){base.$el.animate({left:base.$slideWidth*index*-1+"px"},base.options.speed,base.options.sliderEasing);base.$el.children('.current').removeClass('current').parent().children().eq(index).addClass('current');}else{base.$slides.eq(index).siblings().css('zIndex',50).removeClass('current');base.$slides.eq(index).css('zIndex',100).addClass('current').fadeIn(base.options.speed,function(){base.$slides.not('.current').hide();});}
if(base.options.autoPlay){base.clearTimer();base.beginTimer();}
if(base.options.onSlide&&typeof(base.options.onSlide)=="function"){base.options.onSlide(index);}}}
...
if(base.options.keyboardNavigation){base.$el.click(function(){$('.activePlusSlider').removeClass('activePlusSlider');$(this).addClass('activePlusSlider');});$(window).keyup(function(e){if(base.$el.is('.activePlusSlider')){if(e.keyCode==39){base.nextSlide();}else if(e.keyCode==37){base.nextSlide(false);}}});}
if(base.options.paginationTitle){for(var i=0;i<base.$totalSlides;i++){base.$sliderControls.find('a[rel="'+i+'"]').text(base.$slides.eq(i).attr('data-title'));}}};base.init();};$.plusSlider.defaults={createArrows:true,createPagination:true,paginationBefore:false,paginationWidth:true,paginationTitle:false,displayTime:4000,speed:500,autoPlay:true,keyboardNavigation:true,pauseOnHover:true,sliderEasing:'linear',sliderType:'slider',width:false,height:false,onSlide:null};$.fn.plusSlider=function(options){return this.each(function(){(new $.plusSlider(this,options));});};})(jQuery);