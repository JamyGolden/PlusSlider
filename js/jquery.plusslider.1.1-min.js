...
if(base.options.createPagination){base.$el.after('<div class="plusSlider-controls" />');base.$sliderControls=base.$el.next('.plusSlider-controls');base.$sliderControls.wrap('<div class="plusSlider-controls-wrapper" />');for(var i=0;i<base.$totalSlides;i++){base.$sliderControls.append('<a href="#" rel="'+i+'">'+(i+1)+'</a>');}
base.$sliderControls.width(base.$sliderControls.find('a').outerWidth(true)*base.$totalSlides);if(base.options.sliderType=='slider'){base.$sliderControls.find('a').click(function(){var $this=$(this);if(!base.$slides.is(':animated')){base.$rel=$this.attr('rel');$this.addClass('current').siblings().removeClass('current');base.$el.animate({left:base.$slideWidth*base.$rel*-1+"px"},base.options.speed,base.options.sliderEasing);if(base.options.autoPlay){base.clearTimer();base.beginTimer();}}
...
if(direction==true&&!base.$slides.is(':animated')){if(base.$el.children(':last').is(":visible")){if(base.options.createPagination){base.$sliderControls.find('a:last').removeClass('current').siblings('a:first').addClass('current');}
base.$el.children(':visible').removeClass('current').css('zIndex',60).siblings().css('zIndex',50);base.$el.children(':first').css('zIndex',100).addClass('current').fadeIn(base.options.speed,function(){base.$slides.not('.current').hide();});}else{if(base.options.createPagination){base.$sliderControls.find('a.current').removeClass('current').next().addClass('current');}
base.$el.children(':visible').removeClass('current').css('zIndex',60).siblings().css('zIndex',50).end().next().css('zIndex',100).addClass('current').fadeIn(base.options.speed,function(){base.$slides.not('.current').hide();});}}else if(direction==false&&!base.$slides.is(':animated')){if(base.$el.children(':first').is(":visible")){if(base.options.createPagination){base.$sliderControls.find('a:first').removeClass('current').siblings('a:last').addClass('current');}
base.$el.children(':visible').removeClass('current').css('zIndex',60).siblings().css('zIndex',50);base.$el.children(':last').css('zIndex',100).addClass('current').fadeIn(base.options.speed,function(){base.$slides.not('.current').hide();});}else if(!base.$slides.is(':animated')){if(base.options.createPagination){base.$sliderControls.find('a.current').removeClass('current').prev().addClass('current');}
base.$el.children(':visible').removeClass('current').siblings().css('zIndex',50).end().css('zIndex',60).prev().css('zIndex',100).addClass('current').fadeIn(base.options.speed,function(){base.$slides.not('.current').hide();});}}
...
if(base.options.keyboardNavigation){base.$el.click(function(){$('.activePlusSlider').removeClass('activePlusSlider');$(this).addClass('activePlusSlider');});$(window).keyup(function(e){if(base.$el.is('.activePlusSlider')){if(e.keyCode==39){base.nextSlide();}else if(e.keyCode==37){base.nextSlide(false);}}});}};base.init();};$.plusSlider.defaults={createArrows:true,createPagination:true,displayTime:2000,speed:500,autoPlay:true,keyboardNavigation:true,pauseOnHover:true,sliderEasing:'linear',sliderType:'slider',width:false,height:false};$.fn.plusSlider=function(options){return this.each(function(){(new $.plusSlider(this,options));});};})(jQuery);